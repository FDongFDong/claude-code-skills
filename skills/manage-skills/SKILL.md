---
name: manage-skills
description: 세션 변경사항을 분석하여 검증 스킬 누락을 탐지합니다. 기존 스킬을 동적으로 탐색하고, 새 스킬을 생성하거나 기존 스킬을 업데이트한 뒤 프로젝트 지침을 관리합니다.
# Claude Code 전용 (Codex는 무시)
...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
<!-- 아직 등록된 검증 스킬이 없습니다. 이 스킬을 실행하면 프로젝트에 맞는 검증 스킬을 자동으로 생성합니다. -->
...
**Claude Code**: `AskUserQuestion`을 사용하여 선택지를 제시합니다.
**Codex CLI / 기타**: 선택지를 텍스트로 나열하고 사용자 응답을 대기합니다.
...
2. **사용자에게 스킬 이름 확인**:
   ...
   **Claude Code**: `AskUserQuestion`을 사용하여 이름 선택지를 제시합니다.
   **Codex CLI / 기타**: 이름 후보를 텍스트로 나열하고 사용자 응답을 대기합니다.
...
4. **연관 스킬 파일 업데이트** — 새 스킬 생성 후 반드시 아래 파일들을 업데이트합니다:
   ...
   **4c. `CLAUDE.md` 업데이트 (파일이 존재하는 경우):**
   - `## Skills` 테이블에 새 스킬 행을 추가합니다
   - 형식: `| verify-<name> | <한 줄 설명> |`
   - `CLAUDE.md`가 존재하지 않으면 이 단계를 건너뜁니다

   **4d. `AGENTS.md` 업데이트 (파일이 존재하는 경우):**
   - `## Skills` 테이블에 새 스킬 행을 추가합니다
   - 형식: `| verify-<name> | <한 줄 설명> |`
   - `AGENTS.md`가 존재하지 않으면 이 단계를 건너뜁니다
...
- `CLAUDE.md`: Skills 테이블 업데이트 (파일이 존재하는 경우)
- `AGENTS.md`: Skills 테이블 업데이트 (파일이 존재하는 경우)
...
| `CLAUDE.md` | Claude Code 프로젝트 지침 (파일이 존재하는 경우, 이 스킬이 Skills 섹션을 관리) |
| `AGENTS.md` | Codex CLI 프로젝트 지침 (파일이 존재하는 경우, 이 스킬이 Skills 섹션을 관리) |
